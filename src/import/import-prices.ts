...
async function main() {
  ...
  for (const chain of chains) {
    try {
      ...
      logger.error(`[PRICES] Error importing ${chain} price: ${JSON.stringify(error)}`);
    }
  }
...
async function importChain(chain: Chain, samplingPeriod: SamplingPeriod, oracleId: string | null) {
  ...
  for (const vault of vaults) {
    try {
      ...
      logger.error(`[PRICES] Error importing ${chain}:${vault.id} price: ${JSON.stringify(error)}`);
    }
  }
}
async function importVault(
  ...
  const allOracleIds = [vault.price_oracle.want_oracleId].concat(shuffle(vault.price_oracle.assets));

  for (const oracleId of allOracleIds) {
    if (onlyOracleId && oracleId !== onlyOracleId) {
      logger.debug(`[PRICES] Skipping oracle id ${chain}:${vault.id}:${oracleId}`);
      continue;
    }
    try {
      logger.verbose(`[PRICES] Importing ${chain}:${vault.id}:${oracleId} prices`);
      await importOracleId(chain, samplingPeriod, oracleId);
      logger.verbose(`[PRICES] DONE importing ${chain}:${vault.id}:${oracleId} prices`);
    } catch (error) {
      logger.error(`[PRICES] Error importing ${chain}:${vault.id}:${oracleId} price: ${JSON.stringify(error)}`);
    }
  }
}

async function importOracleId(chain: Chain, samplingPeriod: SamplingPeriod, oracleId: string) {
  ...
  if (lastPriceRow) {
    logger.debug(
      `[PRICES] Last price found for ${chain}:${oracleId}, importing from there: ${JSON.stringify(lastPriceRow)}`
    );
    readStream = streamBeefyPrices(chain, samplingPeriod, oracleId, {
      startDate: new Date(lastPriceRow.datetime.getTime() + 1000),
    });
  } else {
    logger.debug(`[PRICES] Last price NOT found for ${chain}:${oracleId}, importing all data`);
    readStream = streamBeefyPrices(chain, samplingPeriod, oracleId);
  }
