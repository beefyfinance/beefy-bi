...
export function computeIsDashboardEOL(behaviour: ImportBehaviour, eol: boolean, eolDate: Date | null) {
  ...
  if (eol && eolDate === null) {
    ...
    isDashboardEol = eolDate.getTime() < Date.now() - samplingPeriodMs[behaviour.productIsDashboardEolAfter];
  }
